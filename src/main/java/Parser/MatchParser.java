...
public class MatchParser
{
    ...
    private Result FunctionVariable(String s) throws Exception
    {
        ...
        while (i < s.length() && (Character.isLetter(s.charAt(i)) && s.charAt(i)!='E' || ( Character.isDigit(s.charAt(i)) && i > 0 ) )) {
            f += s.charAt(i);
            i++;
        }
    ...
    private Result Degree(String s) throws Exception
    {
        Result current = Low(s);
    ...
    private Result Low(String s) throws Exception
    {
    ...
    private Result processFunction(String func, Result r)
    {
        if (func.equals("sin")) {
            ...
            return new Result(Math.cos(Math.toRadians(r.acc)), r.rest);
                ...
                return new Result(Math.log1p(r.acc), r.rest);
        ...
        } else if (func.equals("arccos")) { return new Result(Math.acos(r.acc), r.rest);}
            else if (func.equals("arctan")) { return new Result(Math.atan(r.acc), r.rest);}
            else{
            System.err.println("function '" + func + "' is not defined");
        }
        return r;
    }
    public double solution(String f,double x) throws Exception {

        MatchParser result = new MatchParser();
        result.setVariable("x",x);
        try {
            double ans = result.Parse(f);
            return ans;

        }catch (Exception e){return 0;}

    }
    public double SolutionForRadInt(String f,double x) throws Exception {
